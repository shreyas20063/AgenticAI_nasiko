"""Recruitment Agent — FastAPI app with A2A JSON-RPC handler.

Serves on port 8001. Exposes:
...
async def lifespan(app: FastAPI):
    logger.info("Recruitment Agent starting on port 8001...")
    get_agent()
    yield


app = FastAPI(title="HRFlow Recruitment Agent", lifespan=lifespan)
...
def get_agent_card():
    ...
    if _agent_card is None:
        ...
        if os.path.exists(card_path):
            ...
            _agent_card = {"name": "HRFlow Recruitment Agent", "status": "active"}
    return _agent_card
