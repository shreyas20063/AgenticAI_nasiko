"""Employee Services Agent — FastAPI app with A2A JSON-RPC handler.

Serves on port 8002. Exposes:
...
app = FastAPI(title="HRFlow Employee Services Agent")
...
def get_agent():
    ...
    if _agent is None:
        ...
        _agent = Agent()
        logger.info("Employee Services Agent initialized")
    return _agent
...
def get_agent_card():
    ...
    if _agent_card is None:
        ...
        if os.path.exists(card_path):
            ...
            _agent_card = {"name": "HRFlow Employee Services Agent", "status": "active"}
    return _agent_card


@app.on_event("startup")
async def startup():
    logger.info("Employee Services Agent starting on port 8002...")
    get_agent()
...
async def handle_a2a(request: Request):
    try:
        ...
        request_id = body.get("id", "unknown") if "body" in dir() else "unknown"
        try:
...
def main(host: str, port: int):
    """Start the Employee Services Agent."""
